...
public class ClosestKValuesBST2 {
	static class Node {
		double val;
		Node left, right;
		Node(double key){
	...
	public static List<Double> getCloseKVal3(Node root, double tgt, int k){
		List<Double> rslt = new ArrayList<Double>();
		
		dpq = new PriorityQueue<Double>();
		count = 0;
		
		inOrderTra3(root, tgt, k);
	...
	private static void inOrderTra3(Node root, double tgt, int k) {
		if (root == null)
			return;
		
		inOrderTra3(root.left,tgt,k);
		if ( count < k ) {
			dpq.add(root.val);
		} else {
			if ( Math.abs(root.val-k) < Math.abs(dpq.peek()-k)) {
				dpq.poll();
				dpq.add(root.val);
			}
		}
		count++;
		inOrderTra3(root.right,tgt,k);
	}
}
